    ...
    static jsonFormatter = {
        format(spacing = "  ") {
            try {
                ...
                if (formatted != formatter_input.value)
                    formatter_input.value = formatted;
            }
            catch (error) {
                ...
                try {
                    var message = error.toString().match(/^(.+?)position /)[0], errorIndex = error.toString().match(/position (\d+)/)[1], errorLine, errorLineIndex = (() => {
