        ...
        */
        setSyntaxHighlighting(state) {
            ...
            document.querySelectorAll("num-text").forEach(editor => {
                if (editor.syntaxLanguage in Prism.languages)
    ...
    static query(identifier = STE.activeEditor) {
        ...
        */
        function getName(section) {
            if ((document.querySelectorAll(`[data-editor-identifier="${identifier}"]:not([data-editor-change])`).length === 0) && (identifier !== STE.activeEditor))
                return null;
            let name = workspace_tabs.querySelector(`.tab[data-editor-identifier="${identifier}"] [data-editor-name]`).innerText;
            if (!section || (!name.includes(".") && section === "base"))
                return name;
            if (section === "base") {
                name = name.split(".");
                name.pop();
        ...
        */
        reset({ confirm: showPrompt = false } = {}) {
            if (!STE.support.localStorage)
                return false;
            if (showPrompt) {
                if (!confirm("Are you sure you would like to reset all settings?"))
                    return false;
            }
...
if (STE.appearance.appleHomeScreen)
    document.documentElement.classList.add("apple-home-screen");
if (STE.environment.touchDevice)
    document.documentElement.classList.add("touch-device");
if (STE.environment.appleDevice)
...
if (STE.support.webSharing)
    document.documentElement.classList.add("web-sharing");
