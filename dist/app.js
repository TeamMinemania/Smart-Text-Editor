...
for (const input of document.querySelectorAll("input:is([type='text'],[type='url'])")) {
...
new ResizeObserver(() => {
    if (!STE.appearance.windowControlsOverlay)
        return;
    app_omnibox.style.setProperty("--device-pixel-ratio", window.devicePixelRatio.toFixed(2));
}).observe(app_omnibox);
for (const option of app_omnibox.querySelectorAll(".option")) {
    option.tabIndex = -1;
    option.addEventListener("mousedown", event => event.preventDefault());
}
const queryParameters = new URLSearchParams(window.location.search);
await (async () => {
    if (STE.environment.fileProtocol)
        return;
    if (window.location.href.includes("index.html")) {
        history.pushState(null, "", window.location.href.replace(/index.html/, ""));
    }
    if (!("serviceWorker" in navigator) || !STE.appearance.parentWindow)
        return;
    ...
    navigator.serviceWorker.addEventListener("message", async (event) => {
        switch (event.data.action) {
            case "service-worker-activated":
                activateManifest();
                break;
            case "clear-site-caches-complete":
                cleared_cache_card.open();
                break;
            case "share-target": {
...
window.addEventListener("blur", () => {
    if (!STE.appearance.parentWindow)
        return;
    for (const menu of document.querySelectorAll("menu-drop[data-open]")) {
        menu.close();
    }
});
document.body.addEventListener("keydown", event => {
    ...
    if (pressed("Escape")) {
        ...
        if (event.repeat)
            return;
        if (STE.activeDialog && !document.activeElement?.matches("menu-drop[data-open]"))
    ...
    if ((controlShift || shiftCommand) && pressed("m")) {
        event.preventDefault();
        if (event.repeat || !STE.activeWidget)
            return;
        if (STE.activeWidget) {
...
for (const menu of app_menubar.querySelectorAll("menu-drop")) {
    menu.addEventListener("pointerenter", event => {
        if (event.pointerType !== "mouse")
            return;
        if (app_menubar.querySelectorAll("menu-drop:not([data-alternate])[data-open]").length === 0 || menu.matches("[data-alternate]") || menu.matches("[data-open]"))
            return;
        menu.opener.focus();
        for (const menu of app_menubar.querySelectorAll("menu-drop[data-open]")) {
            menu.close();
        }
        menu.open();
    });
...
workspace_tabs.addEventListener("keydown", event => {
    if (event.key !== "ArrowLeft" && event.key !== "ArrowRight")
        return;
    if (!workspace_tabs.contains(document.activeElement) || !(document.activeElement instanceof HTMLElement))
        return;
...
scaler.addEventListener("touchstart", event => {
    if (STE.view !== "split" || event.touches.length !== 1)
        return;
    document.body.setAttribute("data-scaling-change", "");
...
preview_base_input.reset = () => {
    ...
    if (!STE.settings.has("preview-base"))
        return;
