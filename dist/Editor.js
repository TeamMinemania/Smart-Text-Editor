    ...
    constructor({ name = "Untitled.txt", value = "", open = true, autoCreated = false, autoReplace = true } = {}) {
        ...
        this.tab.addEventListener("mousedown", event => {
            if (document.activeElement === null)
                return;
            if (event.button !== 0 || document.activeElement.matches("[data-editor-rename]"))
                return;
        ...
        this.tab.addEventListener("contextmenu", event => {
            if (event.target !== this.tab)
                return;
            let editorRename = this.tab.querySelector("[data-editor-rename]");
...
export function setEditorTabsVisibility({ identifier = STE.activeEditor } = {}) {
    ...
    let spacingOffset = 0;
    if ((workspace_tabs.clientWidth < tab.clientWidth) && !obstructedLeft)
        return;
    if (obstructedLeft) {
