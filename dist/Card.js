...
export class Card extends HTMLElement {
    ...
    connectedCallback() {
        if (this.defined || !this.isConnected)
            return;
        this.defined = true;
        this.addEventListener("keydown", event => {
            if (this.getAttribute("data-type") != "dialog" || event.key != "Tab")
                return;
    ...
    open(previous) {
        if (this.matches(".active") && !this.hasAttribute("data-alert-timeout"))
            return this.close();
        if (this.type != "alert") {
            document.querySelectorAll(`.card.active`).forEach(card => {
                if (card.type != "dialog" && card.type != this.type)
                    return;
                card.close();
                if (!card.matches(".minimize"))
                    return;
        ...
        if (this.type == "widget" && card_backdrop.matches(".active"))
        ...
        if (this.type == "alert") {
            ...
            window.setTimeout(() => {
                if (this.getAttribute("data-alert-timeout") != timeoutIdentifier)
                    return;
        ...
        if (this.type == "dialog") {
            ...
            if (!STE.activeDialog && !STE.dialogPrevious) {
                STE.dialogPrevious = document.activeElement;
            }
            ...
            window.setTimeout(() => {
                ...
                if (previous)
                    this.querySelector(`[data-card-previous="${previous.id}"]`).focus();
            }, transitionDuration);
            STE.activeDialog = this;
        }
        if (this.type == "widget")
            STE.activeWidget = this;
    }
    minimize() {
        const icon = this.controls.minimize.querySelector("svg use");
        ...
        if (!this.matches(".minimize")) {
            ...
            if (this.contains(document.activeElement) && document.activeElement != this.controls.minimize)
                this.controls.minimize.focus();
        }
        else {
            this.classList.remove("minimize");
            window.setTimeout(() => {
                if (this.getAttribute("data-minimize-change") == changeIdentifier)
                    this.style.removeProperty("--card-minimize-width");
            }, transitionDuration);
        ...
        window.setTimeout(() => {
            if (this.getAttribute("data-minimize-change") == changeIdentifier)
                this.removeAttribute("data-minimize-change");
            if (workspace_tabs.getAttribute("data-minimize-change") == changeIdentifier)
                workspace_tabs.removeAttribute("data-minimize-change");
        }, transitionDuration);
    }
    close() {
    ...
    */
    ...
    static #catchCardNavigation(event) {
        if (!STE.activeDialog || event.key != "Tab" || document.activeElement != document.body)
            return;
