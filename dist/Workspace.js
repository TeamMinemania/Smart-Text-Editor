...
export function setView(type, { force = false } = {}) {
    if ((STE.orientationChange && !force) || STE.scalingChange)
        return;
    ...
    if (type != "preview")
        window.setTimeout(setEditorTabsVisibility, transitionDuration);
    window.setTimeout(() => {
...
export function setOrientation(orientation) {
    if (STE.orientationChange || STE.scalingChange)
        return;
...
export async function openFiles() {
    if (!STE.support.fileSystem) {
        ...
        const files = results
            .filter(/** @returns { result is PromiseFulfilledResult<{ name: string; value: string; }> } */ (result) => result.status === "fulfilled")
...
export async function saveFile(extension) {
    if (extension || !STE.support.fileSystem) {
        if (!extension)
            extension = STE.query().getName("extension") ?? "";
        ...
        if (identifier === null)
            throw new Error("No editors are open, couldn't save anything!");
        ...
        if (currentName != rename)
            Editor.rename(identifier, rename);
    }
    if (STE.query().tab?.hasAttribute("data-editor-auto-created"))
        STE.query().tab?.removeAttribute("data-editor-auto-created");
    if (STE.query().tab?.hasAttribute("data-editor-unsaved"))
        STE.query().tab?.removeAttribute("data-editor-unsaved");
    refreshPreview({ force: true });
}
/**
...
export function createDisplay() {
    var width = window.screen.availWidth * 2 / 3, height = window.screen.availHeight * 2 / 3, left = window.screen.availWidth / 2 + window.screen.availLeft - width / 2, top = window.screen.availHeight / 2 + window.screen.availTop - height / 2, features = (STE.appearance.standalone || STE.appearance.fullscreen) ? "popup" : "", baseURL = STE.settings.get("preview-base") || null, source = STE.query().textarea?.value ?? "";
    if (baseURL)
        source = `<!DOCTYPE html>\n<!-- Document Base URL appended by Smart Text Editor -->\n<base href="${baseURL}">\n\n${source}`;
    var link = window.URL.createObjectURL(new Blob([source], { type: "text/html" })), win = window.open(link, "_blank", features);
    if (win === null)
        throw new Error("Couldn't create a display window!");
    window.URL.revokeObjectURL(link);
    win.moveTo(left, top);
    ...
    window.setTimeout(() => {
        ...
        if (!win.document.title)
            win.document.title = STE.query().getName();
    }, 20);
}
/**
...
export function refreshPreview({ force = false } = {}) {
    ...
    if (change)
        editor.tab.removeAttribute("data-editor-refresh");
}
/**
...
export function setScaling(event) {
    ...
    if (STE.orientation == "horizontal")
        scalingOffset = (!touchEvent) ? event.pageX : event.touches[0].pageX;
    if (STE.orientation == "vertical")
        scalingOffset = (!touchEvent) ? event.pageY - header.offsetHeight : event.touches[0].pageY - header.offsetHeight;
    if (scalingOffset < scalingRange.minimum)
        scalingOffset = scalingRange.minimum;
    if (scalingOffset > scalingRange.maximum)
...
function removeScaling() {
    if (!document.body.hasAttribute("data-scaling-active"))
        return;
